...
Due to changes in the Anaconda terms of services, we are recommending against using conda for virtual environments. `venv` is a great alternative built into base Python that is straightforward to use and has compatibility with `pip`. `virtualenv`, `pipenv` and `poetry` are also popular options worth consideration that may have additional functionality for specific needs. If you're using R and Python, `renv` is a good option given its emphasis on interoperability. Documentation for using Python with `renv` is available <a href="https://rstudio.github.io/renv/articles/python.html" target="_blank">here</a>.
