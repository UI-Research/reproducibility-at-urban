...
This guide is intended to help R and Python users at the Urban Institute start using virtual environments. This guide assumes familiarity with Git and GitHub (see [this guide](https://ui-research.github.io/reproducibility-at-urban/urbngit/urbngit.html) if you are not familiar with these tools) and is focused on helping folks start using virtual environments without having to meaningfully modify their existing workflows.
...
To set up a virtual environment for an [R project](https://support.posit.co/hc/en-us/articles/200526207-Using-RStudio-Projects), we recommend using `renv`. To install `renv`, use the standard syntax to install R packages from CRAN: `install.packages("renv")`.
