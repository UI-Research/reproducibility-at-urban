...
For R projects, we recommend using `renv`. For Python projects, we recommend using `virtualenv`. These are by no means the only environment managers for R and Python, but we've found these to be the most reliable and user-friendly for the majority of use cases at Urban.
...
Due to changes in the Anaconda terms of services, we are recommending against using conda for virtual environments. `virtualenv` is a great alternative for the following reasons: it works with older versions of Python, has compatibility with `pip`, has been around for awhile, and is lightweight. `pipenv` and `poetry` are also popular options, but they are more complex and have more features than `virtualenv`. If you're using R and Python, `renv` is a good option given its emphasis on interoperability. Documentation for using Python with `renv` is available <a href="https://rstudio.github.io/renv/articles/python.html" target="_blank">here</a>.
...
To set up a virtual environment for a Python project, we recommend using `venv`, which is built into base Python. Note that this may not work with older versions of Python (pre-3.3), in which case you should use [`virtualenv`](https://virtualenv.pypa.io/en/latest/).
...
1.  **Initialize** a new environment (optionally specifying a version of Python and/or a list of packages) using one of the options below. You should replace `my_env` with a descriptive name specific to your project. 
    ...
    ``` python
    python -m venv my_env                                # create empty environment  
    python3.12 -m venv my_env                            # create environment with specific Python version 
    ```
...
3.  **Install packages** (optionally specifying the package version)

    ``` python
    python -m pip install package_name         # install default version of package 
    python -m pip install package_name==1.0.1  # install specific version of package
    ```
...
4.  **Save** a snapshot of the environment to a file called `requirements.txt`.

    ``` python
    python -m pip freeze > requirements.txt
    ```
    
5.  **Share** the snapshot by sending the `requirements.txt` file to GitHub.
...
6.  **Restore** and activate the snapshot of the environment on another computer:
    ...
    This last command assumes you already have a virtual environment set up (following steps 1 and 2) before trying to add the same set of packages.

7.  **Repeat** the process. As you and your collaborators install, update, and remove packages, repeat steps 3-6 to save and load the state of your project to the `requirements.txt` file across computers.
...
We recommend the official `venv` documentation [here](https://docs.python.org/3/tutorial/venv.html).
